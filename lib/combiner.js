...
var config = {
	...
	'vars'         : {}              //extra variables to put into client js file, you can acces this variables directyl without putting it inside vars object
};
...
var combiner = function(options){
	...
	config.files.forEach(function (file) {
		var fullFilePath = config.cwd + config.folder + file;
		if(config.reload){
			listenOnChange(fullFilePath, combiner); 
		}
		fs.readFile(fullFilePath, 'utf8', function (err, data) {
				if (err) {
					...
					data = data.replace(variableMatch, function(pattern, variable){
						...
						return eval(variable) || ''; //eval is bad but sometimes usefull :)
					});
